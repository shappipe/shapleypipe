...
from aipipe_reprod.primitives import ImputerCatPrim, ImputerMeanPrim, OneHotEncoderPrim, LabelEncoderPrim, NumericDataPrim
    ...
    def single_operator_pipe_eval(self, pipeline: list[int]):
        ...
        try:
            ...
            if d['train'].select_dtypes(include=['object']).any().any():
                if self.dataset_name == 'pbcseq':
                    enc = NumericDataPrim()
                else:
                    enc = LabelEncoderPrim()
                d['train'], d['test'], d['target'] = enc.transform(d['train'], d['test'], d['target'])
...
class PermShapley:
    ...
    def run_full_enumerate_algorithm_cate(self) -> List[int]:
        ...
        os.makedirs('aipipe_reprod/shapley/saves/shapley_values/hss', exist_ok=True)
        pd.DataFrame(_shapley_store).to_csv(
            f'aipipe_reprod/shapley/saves/shapley_values/hss/{self.evaluator.dataset_name}_stage_1.csv',
            index=False
        )
...
class ConstrainedPermShapley:
    ...
    def calc_shapley_value(self, slot_pos: int, target_player: int,
        ...
        try:
            ...
            # 3. 求平均
            return total_marginal_contribution / num_valid_samples
    ...
    def run_search(self) -> List[int]:
        ...
        os.makedirs('aipipe_reprod/shapley/saves/shapley_values/hss', exist_ok=True)
        pd.DataFrame(_shapley_store).to_csv(
            f'aipipe_reprod/shapley/saves/shapley_values/hss/{self.evaluator.dataset_name}_stage_2.csv', index=False)
...
class HierarchicalShapley:
    def __init__(self, dataset_name: str, data: DatasetType, pipeline_length: int, 
                 ...
                 num_samples: int = 100, mab_pretrain_sample=None):
        self.dataset_name = dataset_name
        self.data = data
    ...
    def _pretrain_bandits(self, bandits: Dict[str, UCB1_Bandit], sample_size=None):
        ...
        with open(os.path.join(cache_path, f'{self.dataset_name}.json.bak'), 'r') as f:
            cache = json.load(f)
...
def process_dataset(meta, pipeline_length, enable_pretrain, stage2_method, num_samples, rand_seed, mab_pretrain_sample=None):
    ...
    try:
        ...
        with open(f'aipipe_reprod/shapley/saves/mcts/hie_MCTS_75.tsv', 'a') as f:
            f.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\t{dataset_name}\t{num_samples}\t{rand_seed}\t{mab_pretrain_sample}\t{pipeline}\t{pipeline_length+3}\t{acc:.4f}\n")
...
def run_hierarchical_search(use = [], enable_pretrain=True, stage2_method='shapley', num_samples=100, rand_seed=42, mab_pretrain_sample=None):
...
if __name__ == '__main__':
    ...
    run_hierarchical_search(
        use=['abalone', 'ada_prior','avila','connect-4','eeg','google',
            'house_prices','jungle_chess','micro','mozilla4','obesity',
            'page-blocks','pbcseq','pol','run_or_walk','shuttle','uscensus',
            'wall-robot-nav'],
